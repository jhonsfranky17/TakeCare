...
Later migrations: `0003` tightened the missed-dose check from a 30-min grace period / 15-min sweep to a 15-min grace period / 5-min sweep (both matter together — a short grace period masked by an infrequent sweep buys nothing). `0004` added `family_members.relationship`. `0005` added `medicines.notes` (optional, shown on both the Medicines list and today's dose card). `0006` replaced email auth with the anonymous-sign-in "pick your name" flow described above, and unclaimed every row that existed under the old flow (names/relationships preserved) since their `auth_user_id`s pointed at email identities that no longer mean anything. `0007` added the `family_members_release` policy that logout depends on. `0008` fixed `intake_logs.medicine_id`'s FK, which had no `ON DELETE` clause (default `NO ACTION`) — deleting any medicine with dose history (i.e. almost every medicine, since `generate_todays_doses()` creates a row for it daily) failed with a raw FK-violation error. Now `ON DELETE CASCADE`: removing a medicine also removes its `intake_logs` rows, so its History disappears too — there's no orphaned-log state worth preserving instead, since `History.tsx` renders `dose.medicine.name` via that join and would have nothing to show for a log with no medicine behind it. `0009` lowered `refill_threshold_days`'s default from 7 to 3 and backfilled every existing medicine to match — there's no per-medicine UI for this column (`AddMedicineSheet`'s update never touches it, only insert sets it), so changing just the default would have silently done nothing for medicines that already existed.

### Low-stock alerts repeat daily, not once

`check-low-stock`'s gate used to be "have we ever sent this alert" (`low_stock_alert_sent_at === null`) — a family that missed the one push never heard about it again until the medicine ran out completely. It now re-sends once per day for as long as the medicine stays under `refill_threshold_days`, gated on `hoursSinceLastAlert >= 20` instead of `=== null`. The 20h floor (not 24h) is slack for the daily cron's (`0 8 * * *`) own timing drift while still refusing a same-day duplicate if the function is invoked twice. `low_stock_alert_sent_at` still resets to `null` on refill (topping `current_stock` back above threshold in `AddMedicineSheet`'s save), which is what actually stops the daily nagging.
...
`src/ui/ConfirmDialog.tsx` is a real exception to "don't invent variations": it has no `frontend-export/` counterpart, added later for destructive/state-changing confirmations (undo a dose, remove a medicine). It reuses `AddMedicineSheet`'s overlay color/opacity and `tc-rise` animation but centers instead of bottom-anchoring, since a yes/no confirmation reads like a native alert, not a form. It never uses the amber (`--tc-warn-*`) palette even for destructive actions — that palette stays reserved for missed-dose/low-stock signals per the design's own rule — so a destructive confirm uses the `dark` button variant instead. It must render from a screen-level positioned ancestor (the App shell), not a small per-item one — a per-item `position: relative` ancestor would scope the dialog's `position: absolute; inset: 0` overlay to that item's own box instead of the viewport.
